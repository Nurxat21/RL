...
import torch
import numpy as np
from erl.env.Gym import build_env, build_eval_env
from Eval.ReplayBuffer import ReplayBuffer
...
def train_and_evaluate(args, learner_id=0):  # 2021.11.11
    ...
    env = build_env(env=args.env, if_print=False,
                    env_num=args.env_num, device_id=args.eval_gpu_id, args=args, )###Useless code
    if env.env_num == 1:
    ...
    evaluator.save_or_load_recoder(if_save=False)# Useless Code
    ...
    '''init ReplayBuffer after training start'''#Useless Code
